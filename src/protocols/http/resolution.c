...
int guac_http_resolution_reasonable(guac_user* user, int resolution) {
    ...
    return user->info.optimal_resolution == resolution
        || width*height >= GUAC_HTTP_REASONABLE_AREA;

}

int guac_http_suggest_resolution(guac_user* user) {

    /* Prefer HTTP's native resolution */
    if (guac_http_resolution_reasonable(user, GUAC_HTTP_NATIVE_RESOLUTION))
        return GUAC_HTTP_NATIVE_RESOLUTION;

    /* If native resolution is too tiny, try higher resolution */
    if (guac_http_resolution_reasonable(user, GUAC_HTTP_HIGH_RESOLUTION))
        return GUAC_HTTP_HIGH_RESOLUTION;
