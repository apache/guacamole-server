...
static UINT guac_rdp_cliprdr_format_data_response(CliprdrClientContext* cliprdr,
    ...
    /* Heap-allocate the receive buffer to avoid stack overflow when the
     * configured clipboard limit exceeds the default thread stack size */
    int recv_buf_size = clipboard->clipboard->available;
    char* received_data = guac_mem_alloc(recv_buf_size);
    ...
    if (guac_iconv(remote_reader, &input, data_len,
            GUAC_WRITE_UTF8, &output, recv_buf_size)) {
        int length = strnlen(received_data, recv_buf_size);
