...
/**
 * Sends the given Azure AD device code prompt to the given socket as a complete
 * pipe stream (open, blob, end). Used both for the initial prompt and to bring
 * users that join mid-sign-in up to date.
 *
 * @param client
 *     The guac_client associated with the RDP connection.
 *
 * @param socket
 *     The socket to send the prompt over.
 *
 * @param body
