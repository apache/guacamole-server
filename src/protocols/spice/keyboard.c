...
/**
 * The name of the spice-gtk SpiceInputsChannel GObject property which reports
 * the current state of the remote keyboard lock keys (Caps Lock, Num Lock,
 * etc.) as a bitwise OR of SPICE_INPUTS_* flags.
 */
...
int guac_spice_keyboard_update_keysym(guac_spice_keyboard* keyboard,
    ...
    if (key == NULL || (pressed && key->pressed == NULL) || (!pressed && key->pressed != NULL)) {
        ...
        /* Fall back to dead keys if otherwise undefined inside the current
         * keymap (note that we only handle "pressed" here, as dead keys cannot
         * have a pressed/released state). This allows precomposed accented
         * characters (such as "á" or "ñ") to be typed on layouts which produce
         * them via a dead key followed by a base key. */
        if (definition == NULL && pressed) {
            if (guac_spice_decompose_keysym(keyboard, keysym))
                guac_client_log(keyboard->client, GUAC_LOG_DEBUG,
                    "Undefined key will not be sent: 0x%X", keysym);
        }
