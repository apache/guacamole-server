    ...
    while (<INPUT>) {
        ...
        # Map
            ...
            for (my $i=0; $i<=$#keysyms; $i++) {
                ...
                # Encode "extended" scancodes (those requiring a leading 0xE0
                # byte on the wire) using the 0x100 bit, matching the encoding
                # expected by spice-gtk's spice_inputs_channel_key_press().
                my $scancode = $scancodes[$i];
                $scancode |= 0x100 if $ext_flags;
