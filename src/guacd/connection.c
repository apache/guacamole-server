...
static int __write_all(int fd, char* buffer, int length) {
    ...
    while (remaining_length > 0) {
        ...
        int written;
        GUAC_EINTR_RETRY(written, write(fd, buffer, remaining_length));
        if (written < 0)
            return -1;
...
void* guacd_connection_io_thread(void* data) {
    ...
    /* Transfer data from file descriptor to socket */
    while (1) {
        GUAC_EINTR_RETRY(length, read(params->fd, buffer, sizeof(buffer)));
...
static int guacd_route_connection(guacd_proc_map* map, guac_socket* socket) {
    ...
    if (new_process) {
        ...
        if (!add_user_failed) {
            ...
            pid_t wait_result;
            GUAC_EINTR_RETRY(wait_result, waitpid(proc->pid, NULL, 0));
