...
/**
 * Returns the memory address of given rectangle within the mutable image
 ...
 *     which the address of given rectangle should be determined.
 *
 * @param rect
 *     The rectangle to determine the offset of.
 *
 * @return
 *     The memory address of given rectangle within the buffer of the given
 *     layer state.
 */
...
/**
 * Returns the memory address of given rectangle within the immutable (const)
 * image buffer of the given guac_display_layer_state, where the upper-left
 * corner of the given buffer is (0, 0). If the memory address cannot be
 * calculated because doing so would overflow the maximum value of a size_t,
 * execution of the current process is automatically aborted.
 *
 * IMPORTANT: No checks are performed on whether the rectangle extends beyond
 * the bounds of the buffer, including considering whether the left/top
 * position of the rectangle is negative. If the rectangle has not already been
