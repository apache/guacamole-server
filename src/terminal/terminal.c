...
/**
 * Returns true if the given keysym is a cursor editing key (Home, End,
 * Insert, Delete, Page Up, or Page Down).
 */
...
/**
 * Returns true if any modifier key (Shift, Ctrl, Alt, or Meta) is currently
 * held.
 */
 ...
 *
 * Formats:
 *   Home/End (xterm):          ESC [ 1 ; <modifier> H / F
 *   Home/End (linux console):  ESC [ 1 ; <modifier> ~ / ESC [ 4 ; <modifier> ~
 *   Insert/Delete/PgUp/PgDn:   ESC [ <param> ; <modifier> ~
 *                              param: 2=Insert, 3=Delete, 5=Page Up, 6=Page Down
 *
 * Example:
 *   Ctrl+Delete  => ESC [ 3 ; 5 ~
 *   Ctrl+Home    => ESC [ 1 ; 5 H  (xterm)
 *
 * Reference:
 *   https://invisible-island.net/xterm/ctlseqs/ctlseqs.html
 */
 ...
 *             param: 15(F5),17(F6),18(F7),19(F8),20(F9),21(F10),23(F11),24(F12)
 *
 * Example:
 *   Alt+F1 => ESC [ 1 ; 3 P
 *   Ctrl+F6 => ESC [ 17 ; 5 ~
...
static int __guac_terminal_send_modified_function(guac_terminal* term, int keysym) {
    ...
    /* F1-F4 use SS3-style suffix (P/Q/R/S); F5-F12 use CSI tilde with numeric
     * param. The `else return 0` above guarantees that exactly one of suffix
     * or param is non-zero when this point is reached. */
    if (suffix != 0)
...
static int __guac_terminal_send_key(guac_terminal* term, int keysym, int pressed) {
     ...
     *
     * MODE_SWITCH, ISO_LEVEL3_SHIFT (AltGr), and ISO_LEVEL5_SHIFT are treated as
     * Alt, as they behave like Alt on many international/Mac keyboards.
     */
    ...
    /* If key pressed */
         ...
         * directly in CSI (below). For all other keys, Alt is represented by
         * prefixing ESC.
         */
        if (term->mod_alt
        ...
        else if ((keysym >= GUAC_KEYSYM_KP_MULTIPLY && keysym <= GUAC_KEYSYM_KP_9)
                 || keysym == GUAC_KEYSYM_KP_EQUAL) {
